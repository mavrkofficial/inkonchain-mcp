...
- **Fixed `wallet_create` not taking effect until a server restart.** `getPrivateKey()` memoizes the key in-process; `storePrivateKey`/`deletePrivateKey` now invalidate that cache so a freshly created or overwritten wallet is used immediately by the running server. `wallet_create` also flags when `EVM_PRIVATE_KEY` is set, since that env var overrides the keychain (the new wallet won't be used until it's removed).
